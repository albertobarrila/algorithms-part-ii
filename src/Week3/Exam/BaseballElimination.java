    ...
    public Iterable<String> certificateOfElimination(String teamName) {
        ...
        for (int j = 0; j < numberOfTeams; j++) {
            for (int i = j + 1; i < numberOfTeams; i++) {
                ...
                flowNetwork.addEdge(
                        new FlowEdge(gameVertex, getTeamVertex(i), Double.POSITIVE_INFINITY));
                flowNetwork.addEdge(
                        new FlowEdge(gameVertex, getTeamVertex(j), Double.POSITIVE_INFINITY));
                gameVertex++;
            }
        }
        ...
        for (Team t : teams.values()) {
            ...
            int limit = team.getWins() + team.getRemaining() - t.getWins();
            if (limit < 0) {
                return new HashSet<>(Collections.singletonList(t.getName()));
            }

            flowNetwork.addEdge(
                    new FlowEdge(getTeamVertex(t.getId()), TARGET_VERTEX, limit));
        }
